...
function TextList({ items }: { items: string[] }) {
  return (
    <ul className="mt-5 min-w-0 max-w-full space-y-3">
      {items.map((item) => (
        <li key={item} className="flex min-w-0 max-w-full gap-3 text-sm leading-6 text-white/50">
...
export function AiAuditResult({ result, dictionary }: { result: AuditResult; dictionary: AuditDictionary }) {
  return (
    <div className="w-full min-w-0 max-w-full space-y-5">
      <article className="audit-panel relative w-full min-w-0 max-w-full overflow-hidden rounded-[22px] border border-[#a6b3ff]/[0.14] bg-[#0b0d12]/95 p-6 sm:p-8">
        ...
        <div className="relative mt-6 grid min-w-0 max-w-full gap-4 lg:grid-cols-3">
          <section className="min-w-0 max-w-full rounded-2xl border border-white/[0.07] bg-white/[0.02] p-5 lg:col-span-3">
            ...
            <p className="mt-3 min-w-0 text-[15px] leading-7 text-white/68 [overflow-wrap:anywhere]">{result.plainLanguage.currentProcess}</p>
          </section>
          {([
            ...
            <section key={title} className="min-w-0 max-w-full rounded-2xl border border-white/[0.07] bg-white/[0.018] p-5">
      ...
      <div className="audit-result-grid grid min-w-0 max-w-full gap-5 xl:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)]">
      <div className="min-w-0 max-w-full space-y-5">
        <article className="audit-panel min-w-0 max-w-full rounded-[22px] border border-white/[0.08] bg-[#0b0d12]/90 p-6 sm:p-8">
          ...
          <p className="mt-5 min-w-0 text-base leading-7 text-white/66 [overflow-wrap:anywhere] sm:text-lg sm:leading-8">{result.summary}</p>
        </article>
          ...
          <ol className="mt-6 space-y-6">
            {result.automationOpportunities.map((item, index) => (
              <li key={`${item.title}-${index}`} className="grid grid-cols-[28px_1fr] gap-3 border-t border-white/[0.06] pt-5 first:border-0 first:pt-0">
                ...
                <div className="min-w-0 [overflow-wrap:anywhere]"><h3 className="font-medium tracking-[-0.02em] text-white/82">{item.title}</h3><p className="mt-2 text-sm leading-6 text-white/44">{item.description}</p></div>
              </li>
            ))}
          </ol>
        </article>
      ...
      <div className="min-w-0 max-w-full space-y-5 xl:sticky xl:top-24 xl:self-start">
          ...
          <ol className="relative mt-8 space-y-2" aria-label={dictionary.sections.architecture}>
            {result.architecture.map((node, index) => (
              <li key={`${node.label}-${index}`} className="group relative pb-6 last:pb-0">
                ...
                <div className={`relative flex min-h-11 min-w-0 max-w-full items-center gap-4 rounded-xl border px-4 py-3 ${nodeStyles[node.type]}`}>
                  ...
                  <span className="min-w-0 text-sm leading-5 [overflow-wrap:anywhere]">{node.label}</span>
